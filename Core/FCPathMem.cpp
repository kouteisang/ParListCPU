...
void FCPathMem::constructCore(uint** degs, uint *klmd, uint *pos, uint *core, uint n_vertex, uint n_layer, coreNode *node, uint new_e, bool serial, coreNode* father){
    ...
    // }
    // if(father != nullptr)
    // cout << "k = " << node->k << " lmd = " << node->lmd << " len = " << node->length << " father len = " << father->length << endl;
    ...
    if(serial){
        ...
        std::sort(father->core, father->core + father->length);
        std::sort(node->core, node->core + node->length);
        if(diff_len > 0){
            father->diff_res = new uint[diff_len];
            std::set_difference(father->core, father->core + father->length, 
                            node->core, node->core + node->length, father->diff_res);
        }
        delete[] father->core;
    }
...
void FCPathMem::PrintCoreInfor(uint *klmd, uint *core, uint new_e, uint n_vertex){
    // if(klmd[1] == 1){
    cout << "( ";
    cout << " k = " << klmd[0] << " ";
    cout << " lmd = " << klmd[1] << " ";
...
bool FCPathMem::check(uint **degs, uint u, uint* klmd, uint n_layers){
    uint k = klmd[0];
    uint lmd = klmd[1];
    uint cnt = 0;
...
uint FCPathMem::peel(MultilayerGraph &mg, uint **degs, uint* klmd, uint *core, uint *pos, uint s, uint e){
...
// ==========Path Serial==========
void FCPathMem::PathSerial(MultilayerGraph& mg, uint *klmd, uint** degs, coreNode *node, uint *core, uint *pos, uint e, uint &count, coreNode *father){
    ...
    if(n_vertex - new_e > 0){
        ...
        constructCore(degs, klmd, pos, core, n_vertex, n_layer, node, new_e, true, father);
    }else{
        // node = nullptr;
        node->k = 0;
        node->lmd = 0;
    ...
    if(n_vertex - new_e > 0 && klmd[0] == 1){
        ...
        klmd[1] += 1;
        PathSerial(mg, klmd, degs, rightChild, core, pos, new_e, count, node);
...
// ==========Path Parallel==========
void FCPathMem::PathParallel(MultilayerGraph &mg, coreNode *node, uint** degs, uint *pos, uint *core, uint e){
   ...
   if(n_vertex - new_e > 0){
        ...
        // cout << "???" << endl;
        // PrintCoreInfor(klmd, core, new_e, n_vertex); 
        constructCore(degs, klmd, pos, core, n_vertex, n_layer, leftChild, new_e, false, node);
...
// ==========Execute==========
void FCPathMem::Execute(MultilayerGraph &mg, FCTree &tree){
    ...
    #pragma omp parallel
    {
        #pragma omp single
        {
            PathSerial(mg, klmd, degs, node, core, pos, e, count, nullptr); 
        }
        #pragma omp taskwait
    }
