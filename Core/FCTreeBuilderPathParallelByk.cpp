#include "FCTreeBuilderPathParallelByk.h"
// #include "FCTree.h"

FCTreeBuilderPathParallelByk::~FCTreeBuilderPathParallelByk(){
}

// ==========common method==========

void FCTreeBuilderPathParallelByk::constructCore(uint** degs, uint *klmd, uint *pos, uint *core, uint n_vertex, uint n_layer, coreNode *node, uint new_e, bool serial){
...
void FCTreeBuilderPathParallelByk::PrintCoreInfor(uint *klmd, uint *core, uint new_e, uint n_vertex){
    cout << "( ";
    cout << " k = " << klmd[0] << " ";
    cout << " lmd = " << klmd[1] << " ";
    cout << ") len  = ";
...
bool FCTreeBuilderPathParallelByk::check(uint **degs, uint u, uint* klmd, uint n_layers){
    uint k = klmd[0];
    uint lmd = klmd[1];
    uint cnt = 0;
...
uint FCTreeBuilderPathParallelByk::peel(MultilayerGraph &mg, uint **degs, uint* klmd, uint *core, uint *pos, uint s, uint e){
...
void FCTreeBuilderPathParallelByk::PathSerial(MultilayerGraph& mg, uint *klmd, uint** degs, coreNode *node, uint *core, uint *pos, uint e, uint &count){
...
void FCTreeBuilderPathParallelByk::PathParallel(MultilayerGraph &mg, coreNode *node, uint** degs, uint *pos, uint *core, uint e){
